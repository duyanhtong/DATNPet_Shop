...
export class CartItemService {
  ...
  async getList(user: User, filter: CartItemFilterType): Promise<any> {
    ...
    query.andWhere(
      new Brackets((qb) => {
        qb.orWhere('cart_item.order_id IS NULL');
      }),
    );
  ...
  async countCart(user: User): Promise<any> {
    const query = this.cartItemRepository.createQueryBuilder('cart_item');
    query.andWhere(
      new Brackets((qb) => {
        qb.where('cart_item.user_id  =:user_id', { user_id: user.id });
      }),
    );
    query.andWhere(
      new Brackets((qb) => {
    ...
    const totalCart = await query.getCount();
    return totalCart > 99 ? 99 : totalCart;
  }
