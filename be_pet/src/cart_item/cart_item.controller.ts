...
export class CartItemController extends BaseController {
  ...
  async create(
    ...
    try {
      console.log('run api add product to cart');
  ...
  @Get('count')
  @Roles('admin', 'user')
  async countTotal(@Request() req: any): Promise<any> {
    try {
      console.log('run api count total cart item');
      const result = await this.cartItemService.countCart(req.user);
      if (result instanceof CommonError) {
        throw result;
      }
      return this.data(result);
